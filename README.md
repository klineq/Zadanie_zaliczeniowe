...
## Jak wrzucić projekt na Git (GitHub / GitLab)

W folderze projektu jest `.gitignore` — ignoruje m.in. `bin/`, `obj/`, pliki Visual Studio i zapis gry `spacebase_save.json`.

### 1. Pierwszy raz (nowe repozytorium lokalne)

```powershell
cd c:\Users\Muzeum\Desktop\zadanie_klinek

git init
git add .
git status
git commit -m "Symulator logistyki bazy kosmicznej - projekt PO"
```

`git status` powinien pokazać kod źródłowy (`*.cs`, `*.csproj`, `*.sln`, `README.md`, `.gitignore`) — **bez** folderów `bin` i `obj`.

### 2. Podpięcie zdalnego repozytorium (np. GitHub)

1. Na GitHubie: **New repository** → np. `space-base-logistics` → **bez** README (masz już lokalnie).
2. Skopiuj URL, np. `https://github.com/TWOJ_LOGIN/space-base-logistics.git`

```powershell
git branch -M main
git remote add origin https://github.com/TWOJ_LOGIN/space-base-logistics.git
...
| Ignorowane | Po co |
|------------|--------|
| `bin/`, `obj/` | wynik kompilacji — odtwarza się przez `dotnet build` |
| `.vs/`, `*.user` | ustawienia IDE |
| `spacebase_save.json` | lokalny zapis z gry |
